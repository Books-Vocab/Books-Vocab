...
const BATCH = [
  { surface: 'Podcast Hero', dir: 'podcast-hero', scenarios: ['A11y3', 'Episodes only', 'Fresh · no meta', 'Full meta', 'Long title · multi host'] },
  { surface: 'Podcast · Bubble Cell', dir: 'podcast-bubble-cell', scenarios: ['Highlighted (active)', 'Idle (non-current)', 'Right-aligned speaker', 'Vocab highlighted'] },
  { surface: 'Podcast Continue Rail Card', dir: 'podcast-rail-card', scenarios: ['A11y3', 'Large numbers', 'Long title', 'No progress', 'Resume'] },
]
...
STEPS:\n  TITLE AUTHORITY: scenario strings handed to you may underscore-encode special separators. The AUTHORITATIVE catalog title is the exact string in the *Scenarios.swift scenario registration you read in step 1 — the PNG filename is that string after slugify ([./:\\s/] and middot-handling; the middot char is PRESERVED, slash/space/colon/dot become _). Your manifest ref scenario MUST be that exact Swift source string (NOT your underscore-decoded guess) so ios-ref slugify round-trips to the real PNG. ls the catalog dir to confirm.\n1. iOS SoT: grep ios/BooksAndVocab/Debug/Scenarios/ for your surface to find the *Scenarios.swift; read each scenario's props/layout. Then grep ios/BooksAndVocab for the component struct(s) and read the view tree + modifiers + appSkin tokens.
