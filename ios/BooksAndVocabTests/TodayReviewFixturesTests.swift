...
@Suite(.serialized) struct TodayReviewFixturesTests {
    ...
    @Test func repoAndGeneratedDatasetsDeclareNoUnknownTodayReviewFixtureKey() throws {
        ...
        #expect(!repoDocument.todayReview.isEmpty)
        #expect(Set(repoDocument.todayReview.keys).isSubset(of: expected))
        #expect(!generatedDocument.todayReview.isEmpty)
        #expect(Set(generatedDocument.todayReview.keys).isSubset(of: expected))
    }
