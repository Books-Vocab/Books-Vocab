...
外部 scheduler 只能用 `watchdog-claim` 的 `verdict=wake-authorized`、`action=wake` 且 `wake_claimed=true` 結果喚醒 Supervisor；shell caller 也必須把 exit 0 視為唯一 dispatch 授權。`watchdog` 保留給唯讀觀測。收到 `wake` 但未取得 claim、`noop` 或 `escalate` 時，scheduler 必須結束本 tick，不建立 Codex session；`verdict=no-wake`／exit 2 是正常的防重複結果，不是重試或建立 session 的理由。
