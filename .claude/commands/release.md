...
1. **看待發版**：跑 `./ops/release.sh status`。若目標是 iOS，再跑 `./ops/asc.sh versions` / `review-status`：先判斷上一個 marketing version 是否已完成審查，再談新版本號；git 的 semver 建議不能取代 ASC lifecycle 真相。
...
4a. **tag-only**（原名 `publish`，別名保留；dry-run 預設）：先**不帶 --yes** 跑一次給使用者看計畫：
   ```bash
   ./ops/release.sh tag api <x.y.z>
   ./ops/release.sh tag ios <x.y.z> --new-version-after-ready <previous>
   ```
...
4b. **release**（三平面統一發布，**碰生產、須使用者明確同意**；dry-run 預設）：
   ...
   ./ops/release.sh release ios <x.y.z> --new-version-after-ready <previous>
   ./ops/release.sh release ios <x.y.z> --new-version-after-ready <previous> --yes
   ```
   ...
   - 執行 `release` 前不可先跑 4a tag-only；否則 release 會因 tag 已存在而拒絕。
