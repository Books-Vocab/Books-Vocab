...
| 腳本 | 用途 |
|------|------|
| `ios_ops.sh` | iOS ops 統一入口(agent 優先用):`status`(`--json` schema `kg.ios.status.v1`,只回 project/Organizer/TestFlight quick summary)、`build`/`test`/`archive`(委派 primitives；一般 `build --json` / `test --json` 由 façade 執行 delegate 後回傳 `kg.ios.run.v1`，內嵌 `kg.ios.diagnostics.v1`；`archive --json` 則回 `kg.ios.archive.v1`，也帶 diagnostics，並拆出 `archive/export/upload` 三段語意；`test --cache-status|--prepare-cache|--clean-cache --json` 保留原生 `kg.ios.test-cache.v1`)、`archives`(Organizer `.xcarchive` 查詢)、`issues`(xcodebuild log diagnostics)、`logs`(runtime log show + 噪音過濾;預設 `--since` 回溯快照,`--json` schema `kg.ios.logs.v1`,資料源 `/usr/bin/log show --style ndjson`,含 raw/filtered/emitted counts、byEventType 與 entries;`--follow` 即時串流 `/usr/bin/log stream`,`--follow --json` 逐行輸出 `kg.ios.log-stream.v1`,`--limit` 在 follow 模式為 stop-after-N gate)、`sentry`(iOS Sentry wiring 摘要;`--json` schema `kg.ios.sentry.v1`,固定回 source path/existence、`canImport(Sentry)` guard、DSN/debug test arg wiring、release name/dist contract)、`doctor`(read-only release readiness:project/Organizer/TestFlight/ASC version state/signing/StoreKit/Sentry,輸出 `[ios][readiness] ... status=ok|warn|block` 與 `[ios][doctor] summary verdict=...`;`--json` schema `kg.ios.doctor.v1`，除 `readiness[]` 外也直接內嵌同一份 `sentry: kg.ios.sentry.v1`，並提供 `summary.verdict/counts.ok|warn|block|total`；ASC state 有短 deadline)、`workflow release`(read-only 發版步驟編排,輸出下一步 command 與 todo/ready/block/warn/manual，文字模式另印 `[ios][workflow] summary ...`;`--json` schema `kg.ios.workflow.v1`，含 `summary.verdict/counts.ready|todo|block|warn|manual|total`)、`gate release`(read-only release hard-stop verdict;`--json` schema `kg.ios.gate.v1`;exit code `0=pass`/`1=warn`/`2=block`,todo/manual 不讓 gate 失敗)、`xcode`/`environment`(read-only Xcode/project/destination/simulator inventory;`--json` schema `kg.ios.xcode.v1`,組合 `xcodebuild -version`/`-list`/`-showdestinations` + `simctl list devices`,destinations 分 `available[]`/`ineligible[]`,來源失敗保留 `sources` + `errors[]`)、`simulator`/`sim`(status 唯讀查 booted simulator + app data container + app process `running|stopped|skipped|unknown`;app process probe 改走 host-side `ps -axo pid,command` 並以 device UDID + `BooksBrowser.app/BooksBrowser` 鎖定,避免 simulator runtime `pgrep/sysmond` 不穩；`ensure-booted` 會重用既有 booted simulator,否則 boot 預設裝置並等待 `bootstatus`;launch/terminate 是本機 Simulator lifecycle side effect,底層 `xcrun simctl launch|terminate`,完成後重新讀 process state;不 build/install;screenshot 只產生本機 PNG artifact;`--json` schema `kg.ios.simulator.v1`;所有 simulator action 皆帶 `timings`，可拆 `simctlDevicesMs/appContainerMs/appProcessMs/resolveMs/bootMs/bootstatusMs/lifecycleMs/screenshotMs/totalMs`)、`test --cache-status|--prepare-cache|--clean-cache`(顯式管理 test `build-for-testing` cache 與 `.xctestrun` reuse;`--json` schema `kg.ios.test-cache.v1`,含 scope/scheme/destination/cache root/derived data/xctestrun/productsReady/timings)、`catalog prepare|snapshots|clean`(Playbook catalog 截圖控制面;`prepare` 預熱 build cache,`snapshots` 支援 `--group`/`--scenario` scope 與 `--reuse-build`,也可用 `--dataset <name>` 或 `--dataset-file <path>` 將外部 fixture dataset staged 到 simulator `/tmp/kg-fixture-dataset.json`,讓 catalog/preview 走資料注入而不改 iOS 畫面程式碼;`--json` 會回 dataset/cache/timing/validation 狀態)、`runs`/`reports`(read-only 最近 build/test/archive verdict + log/xcresult artifact path + 內嵌 `kg.ios.diagnostics.v1`;`--json` schema `kg.ios.runs.v1`;頂層另有 `summary.verdict` 與 `summary.counts.errors|warnings|failedTests|missing|malformed|failing`，文字模式第一行固定 `[ios][runs] summary ...`；每個 run 會保留 `options` / `cache` / `timings`，archive 另含 `lockWaitMs/archiveMs/exportMs/uploadMs/totalMs`)、`snapshot`/`dashboard`(read-only 一次拉 project/Organizer/TestFlight/readiness/workflow/gate/sentry/xcode/simulator/runs;JSON 與文字模式共用同一份 `kg.ios.snapshot.v1` payload formatter,文字第一行固定 `[ios][summary]`，後面緊接 `[ios][timing] build ...` / `[ios][timing] test ...` / `[ios][timing] archive ...` / `[ios][timing] simulator ...` 再列 `[ios][next]`;頂層 `summary.verdict/counts/nextActions/timings` 是第一屏判讀層,其中 `summary.timings.simulator` 直接提升 `kg.ios.simulator.v1.timings`，`summary.counts.sentryWarnings` 直接 surfaced wiring drift，且 `summary.counts` 也直接提升 `doctor.summary.counts` / `workflow.summary.counts` 成 `readinessOk|readinessWarns|readinessBlocks|workflowReady|workflowTodos|workflowWarns|workflowBlocks|workflowManual`；`summary.nextActions` 亦包含 gate `manual` steps。snapshot 內的 `sentry` 直接重用 `doctor.sentry`，不再另走一條判讀路徑；預設內嵌 `kg.ios.workflow.v1` + `kg.ios.gate.v1` + `kg.ios.sentry.v1` + `kg.ios.xcode.v1` + `kg.ios.simulator.v1` + `kg.ios.runs.v1` + `runs.*.diagnostics`;`--skip-xcode` 回 `xcode:null`;`--skip-simulator` 回 `simulator:null`;no booted simulator 會內嵌 `.simulator.status=error` 但 snapshot exit 0;`--include-logs` 再內嵌 `kg.ios.logs.v1`,預設 `logs:null`)、`commands`/`capabilities`(read-only 自描述 catalog;`--json` schema `kg.ios.commands.v1`,列 sideEffect/delegate/schema，且 `jsonSchemas[]` 需同步列出穩定內嵌 child schema，不只 top-level)。高風險 upload 仍只在 `archive --upload` 明示時發生。測試 `ops/test_ios_ops.sh` + `ops/tests/test_ios_diagnostics.py` |
| `capture_profile.py` | capture/行銷編排層：把 `ops/devops_kg_safe.sh ops-edit ...` 的 demo 資料造景（`seed`、`user-config-set`、`notebook-update --sort-order`）與 `ops/ios_ops.sh catalog snapshots --dataset-file ...` 綁成單一 profile/workflow。profile schema:`kg.capture.profile.v1`，描述 `materialize(uid, seedFile, steps[])` + `snapshot(datasetFile, destination, groups/scenarios)`；runner schema:`kg.capture.run.v1`，支援 `plan` / `materialize` / `snapshot` / `run`。安全模型:預設 dry-run，不加 `--commit` 不會讓 `ops-edit` 真寫入；snapshot 仍只做本機 artifact。範例 profile:`ops/capture_profiles/marketing_demo.json` |
| `ios_diagnostics.py` | iOS diagnostics adapter:`xcresulttool get build-results` 為 build/archive 優先資料源,抽官方 `.xcresult` 的 `errorCount`/`warningCount`/issues;`--kind test` 用 `xcresulttool get test-results summary/tests/metrics` 抽 executed/failures/failing tests、`testBodyMs`/`xcresultSessionMs`，以及 `XCTApplicationLaunchMetric` 的 `AppLaunch average/min/max/samples`;`--kind archive` 與 build 共用 build-results surface；raw xcodebuild log parser 只作 fallback,分類 Swift 6 concurrency、StoreKit config、SPM、signing。文字輸出第一屏 summary,`--json` 給 agent/CI。`ios_build.sh` / `ios_test.sh` / `ios_release.sh` 已接線 |
...
| `test_ops.sh` | ops regression 聚合入口。預設非 ASC suite:`release` / `ios-release` / `backup-verify` / `devops` / `deploy-smoke` / `infra-health` / `branch-audit` / `python-entrypoints` / `ui-token` / `docs-lint` / `ios-ops` / `ios-test-discovery` / `chrome-bundle` / `podcast-ops`;optional release surface:`asc`(ASC shell + text bundle offline tests) / `release-surfaces`(`release`+`ios-release`+`asc`)。也可指定 group,`--list` 列清單。`branch-audit` 用離線 fixture 驗證分類與 JSON,不依賴 live GitHub 狀態；`python-entrypoints` 掃全 `ops/*.py`，禁止裸 `python3` shebang、要求 executable Python entrypoint 使用 uv shebang、有 shebang 即須有 executable bit |
...
Container 內 ops-cli(`card-find`、`db-query`、`llm-errors`、`user-config <uid>`（唯讀檢視 users.json 的 user config:translation / review_clock / review_mode / **vocab_ui** active notebook,active notebook 後端化）、`ops_analyze.py` levels 1-6 等)由 `devops` skill 包裝呼叫。傳輸層以 `printf %q` 序列化 argv,任意特殊字元 SQL 可安全穿越單次遠端 bash 解析。

**寫入面**:`backend/ops_edit.py`(`devops.sh ops-edit` / `devops_kg_safe.sh ops-edit`)為 `ops_cli.py` 唯讀查詢的可寫對應面 —— 建用戶 / 增改刪卡 / 設複習態 / 搬卡(`card-move`)/ 筆記本 CRUD(`notebook-create`/`update`/`delete`; `notebook-update` 支援 `--sort-order` 做 surface 排序造景)/ 連結圖譜(`link-add`/`update`/`delete`/`list`)/ `user-config-set`(寫 `users.json` 的 per-user `translation` / `review_clock` / `review_mode` / `vocab_ui.active_notebook_id`，供 Settings / active notebook 等行銷畫面定向造景)/ `seed` 整套 demo 帳號(冪等可重跑;支援頂層 `review_anchor` 與單卡 `review.anchor` 固定複習時鐘,`source` 以 `VocabSource` schema 驗證後寫入)/ `clone-demo <source_uid> <target_uid>` 高保真複製真帳號 vocab 層(cards/notebooks/graph/embeddings/candidates,SQLite 走 online backup API、衍生檔 copy2)+ 由複習聚合(`review_count`/`lapse_count`/`review_streak`/`last_reviewed_at`)確定式合成 `review_events.db`(餵 iOS heatmap/streak;`backend/src/kg/demo_review_synth.py`),目標 identity(`users.json`,data-root 非 user_dir)結構上不動 / `restore` 還原。內建行銷 seed:`ops/seeds/marketing_demo.json`(3 本 notebook / 12 cards / 6 links / 4 new + 4 due + 4 reviewed 的固定展示分布，適合 Today Review / notebook / graph 截圖)。`--notebook`/`--to-notebook`/`--active-notebook` 接受 id 或 name(自動解析,杜絕孤兒卡);link 嚴格 per-notebook(兩端 card 須與 link 同本,`card-move` 搬卡會硬刪原本跨本 link、`notebook-delete` 非空須 `--cascade`)。安全模型(`backend/src/kg/ops_edit_shared.py:EditContext`):**dry-run 預設**(`--commit` 才寫)、寫前自動 tar 備份 user_dir 到 `data_dir/_ops_backups/`、uid path-safe guard(擋 `..`/`.` 起首)、寫後讀回 verify(link 讀盤繞快取、複習態驗時間不變量、user config 讀回 users.json 驗證)、append `_ops_edit_audit.jsonl`(含失敗操作);`restore` 驗備份 arcname==uid 才解壓。寫入複用 app 的 `CardStore`/`GraphStore`/`NotebookStore`(SoT,不重刻 NFC/dedup/graph merge);只 import per-user store,不碰會自寫的全域 log 單例。同 `%q` argv transport、同 `--json` 契約。
