...
`/api/dictionary/search` 與 `/api/dictionary/entries/*` 是純 GET integration：完全不經 LLM、不寫 `token_usage`、不扣 daily quota，也不觸發 Card/pipeline/graph 寫入。FreeDictionary（Wiktionary，CC BY-SA 4.0）的成本是上游配額而非金錢——1000 calls/hour/IP，我方以 `lexical_provider_request` 原子預約 + 950 上限自我節流，並用跨用戶 `lexical_cache.db`（正向 30 天／負向 24 小時）攤薄。因此字典查詢量變化不會出現在 §1／§2 金額表；上游配額、cache 與 lookup 健康看 `ops_cli.py dictionary-health`。
