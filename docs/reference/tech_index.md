...
| `catalog_review_entry.py` | catalog review artifact 入口：`current` 從 `build/snapshots/*/review_manifest.json` 掃描現有 review desk，選出 blessed artifact（先比 `totalImages`，再以名稱打破同分）並輸出 `reviewHtml` / `reviewManifest` / `promiseCounts` / `heroCandidates` 等摘要；`serve [--port]` 會先停掉同 port 既有 listener，再以 blessed artifact root 啟本機 `http.server` 直接服務 `review.html`。適合 agent/human 在多份 catalog 輸出並存時先解「該看哪份」與「本機快速預覽」兩個問題。測試：`ops/tests/test_catalog_review_entry.py` |
