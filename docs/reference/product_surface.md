...
- Chrome 加詞 outbox：content popup 加詞先寫入 `chrome.storage.local.vocab_outbox`，sidepanel 立即顯示該 notebook 的 pending/failed optimistic rows；背景 worker 依 notebook 分批 sync 到 `/api/vocab?notebook_id=...`，成功後觸發同 notebook enrich pipeline，失敗則標記「待重試」並用 alarm 週期喚醒重送。
