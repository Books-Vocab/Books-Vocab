...
10. Mutation subcommands are dry-run by default and use `--commit` to land, except `backlog.py add`:一般 filing stays immediate by default, with explicit `--dry-run` preview and `--commit` compatibility alias；wave worker 必須用 `add --stage` 延後到整合者 anchor，避免把自己的 ticket filing 混進修復 diff。Backlog role/state semantics live in `backlog.py lifecycle --json`.
