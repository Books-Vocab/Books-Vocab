...
def cmd_supersede(args: argparse.Namespace) -> int:
    """Persist one exact superseded-by-merged-PR proof."""

    if not args.branch and not args.path:
        print("✗ supersede needs --branch/--path", file=sys.stderr)
        return EXIT_USAGE
    if args.expected_generation is None or args.expected_head_sha is None:
        print(
            "✗ supersede requires exact generation and HEAD compare-and-swap guards",
            file=sys.stderr,
        )
        return EXIT_USAGE
    try:
        proof_body = json.loads(args.superseded_proof)
    except json.JSONDecodeError as error:
        print(f"✗ superseded proof is invalid JSON: {error.msg}", file=sys.stderr)
        return EXIT_USAGE
    if not isinstance(proof_body, dict):
        print("✗ superseded proof must be a JSON object", file=sys.stderr)
        return EXIT_USAGE
    target = state_path(args)
    with ledger_lock(target):
        state = load_state(target)
    ...
    print(
        json.dumps(payload, indent=2, ensure_ascii=False)
        if args.json
        else f"✓ superseded proof recorded [{result.record.get('branch')}]"
    )
    return EXIT_OK
