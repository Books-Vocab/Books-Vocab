...
def test_is_synthetic_and_srs_default_when_absent(tmp_path):
    ...
    store = ReviewEventStore(tmp_path / "r.db")
    e = ReviewEventEntry(
        event_id="x",
        card_id="c",
        word_snapshot="w",
...
def test_migration_adds_columns_to_legacy_db(tmp_path):
    ...
    store = ReviewEventStore(path)  # 開啟即觸發 migration
    cols = {row[1] for row in sqlite3.connect(path).execute("PRAGMA table_info(reviewevent)")}
    assert {
        "interval_before",
        "interval_after",
        "next_review_before",
