...
- **合併後的 gate 才回答得了該問的問題。** 每個被派者各自 gate,證明的是「我的改動在我 fork
...
- **順序與衝突是全域決定。** 哪個順序衝突最少、共用檔(如 `docs/reference/tech_index.md`)的
  衝突該怎麼解,需要同時看到全部 N 份 diff。被派者結構上看不到,不是它不夠盡責。
...
**task brief 要寫明**:邊界一節加「在自己的工作樹 commit;**不要**跑 `gate` / `cutover` /
`sync` / `deploy`;完成後回報分支名與工作樹路徑」。單一被派者、單一工作樹、非批次的任務,
可讓它自己跑 `gate`(自驗),但 `cutover` 一律留給整合者——保持動作與落地決定分離。
