...
| IMP-0032 | 2026-07-14 | App Review 2.0.0 evidence producer | cli | med | triaged | `app_review_evidence.py desired` 以 `capture_output=True` 串接 shape / build / `capture_profile.py reviewer-evidence`；最後一段長時間無 phase/heartbeat，操作者無法判斷仍在工作或已卡死。下一步：stderr 加 phase-start + 週期 heartbeat、stdout 維持單一 JSON，並補 stdout purity / progress / exit-code 迴歸。 | — |
| IMP-0033 | 2026-07-14 | App Review 2.0.0 Release iphoneos compile gate | cli | med | triaged | `ios_ops.sh test --ui --configuration Release --destination generic/platform=iOS --prepare-cache --json` 曾在 xcodebuild exit 0、iphoneos xctestrun 與兩個 `.app` 都存在時，同份 JSON 回 `status:"prepared"` 與 `productsReady:false`，machine verdict 自相矛盾。下一步：ready predicate 改為 destination-aware，且 `productsReady=false` 時禁止報 prepared，補 iphoneos fixture regression。 | — |
