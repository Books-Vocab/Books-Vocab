...
| `SettingsPresenter+Quota.swift` | 54 | 額度進度列與色調邏輯（extension 掛 `SettingsOtherSection`，stack 約束）|
| `SettingsPresenter+Debug.swift` | 119 | `SettingsDebugBackendSection` child struct（DEBUG 後端切換 + 觀測事件預覽）|
