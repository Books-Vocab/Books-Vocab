...
| `Services/` | 後端通訊(`KGService`)、認證、雲同步、analytics、sentry;`KGServing` 拆窄協定 `BackgroundSyncing`(`backgroundSync`+`lastBackgroundSyncError`),`ExplicitSync` 為顯式同步(pull-to-refresh/toolbar/⌘R)回饋政策單一真相(成功 toast、失敗 warning + read-then-clear) |
