...
def test_default_kinds_orphan_set():
    ...
    # ordered by (totalRefs asc, name): 0 refs, then 1 (test), then 2 (debug)
    assert _names(orphans) == ["ProdOrphanNoRefs", "ProdOrphanTestOnly", "ProdOrphanDebugOnly"]
...
def test_custom_nonprod_markers():
    ...
    orphans = ui_deadcode.classify_orphans(_records(), nonprod_path_markers=("Tests/",))
    names = _names(orphans)
    assert "ProdOrphanDebugOnly" not in names  # debug refs now count as prod
    assert "ProdOrphanTestOnly" in names       # test refs still excluded
