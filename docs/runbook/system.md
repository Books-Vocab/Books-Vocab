...
**Fan-out scope is an agent ordering rule, not a backlog contract.** When choosing work to fan out, prefer bounded bug fixes, refactors, tooling friction, documentation, and test-maintenance tasks whose fix site and acceptance are independently understandable. Hold new product behavior, strategy, open-ended discovery, and broad cross-cutting product changes for an explicit parent decision. This only guides sorting／派工；it does not change `backlog.py` lifecycle, ticket fields, acceptance semantics, status semantics, or closure rules.
