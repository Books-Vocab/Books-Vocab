...
- **Tests**（`backend/tests`）：API contract、robustness、admin/test-matrix、auth provider tests（Apple/Google）、text_utils/enrich modules、pipeline lock-queue concurrency tests
...
- **Claude Code Gateway**（`lab/claude-code-gateway/`，.gitignore，third-party）：Claude Code CLI → OpenAI-compatible `/v1/chat/completions`，公網 `https://wordnexus.lol/claude/v1`（Caddy `/claude/*` → port 8090），Bearer `CCG_API_TOKEN`，模型別名 `sonnet`/`opus`/`haiku`，目前供 `lab/podcast_architect_poc.py` 使用，詳見 `docs/ops/claude-code-gateway.md`
