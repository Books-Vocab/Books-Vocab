...
- **App Store Connect 查詢/改文案**(`ops/asc.sh`): 純 codemagic CLI 包裝;`versions`/`builds`/`metadata`/`info`/`review-status` 唯讀查版本與審查狀態,`set <field> <value>` 改 version-localization 文案(description/keywords/urls/whats-new/promo,預設 dry-run、`--yes` 才真寫、空值被擋);被拒原因 Resolution Center 文字 API 不可讀(須 GUI);不做 submit-for-review。細節見 `docs/sop/ios.md §發版`
