...
| `catalog_review_entry.py` | catalog atlas artifact 入口：`current` 從 `build/snapshots/*/review_manifest.json` 掃描現有 artifact，先偏好 `isUsable=true`，再以**名稱時間序**選出最新 blessed，輸出 `canvasHtml` / `reviewManifest` / `promiseCounts` / `heroCandidates` / `staleArtifacts` / `supersededArtifactCount` 摘要；`serve [--port]` 會先停掉同 port 既有 listener，再以 blessed artifact root 啟本機 `http.server` 直接服務 `catalog.html`（`url` 直指 canvas）；`prune-stale [--dry-run]` 可刪掉 `totalImages=0` 的空殼；`prune-superseded [--dry-run]` 可刪掉較舊但仍可用的 artifact，只保留最新 blessed 一份。適合 agent/human 在多份 catalog 輸出並存時先解「該看哪份」與「本機快速預覽」兩個問題。測試：`ops/tests/test_catalog_review_entry.py` |
| `catalog_review_cli.py` | catalog atlas 純讀導覽 CLI（接 `review_manifest.json` 的 `tree` 欄位，與 `catalog.html` 共用同一棵節點樹）：`tree [--node <path>] [--depth N]` 列節點樹（每節點都帶 `childCount`，截斷後 schema 一致）、`node <node-path>` 印單一節點 metadata + ancestors + 直系 children、`node-url <node-path> [--base <url>]` 回傳 canvas deep link（預設指向 artifact 內 `catalog.html` 的 file:// URI，可用 `--base http://127.0.0.1:8787/catalog.html` 切到 entry serve 出來的 URL）。`nodePath`（`feature/surfaceKey/stateKey/assetID`）是 canvas 與 CLI 共用的單一坐標，agent 與 human 用同一字串引用同一張圖。測試：`ops/tests/test_catalog_review.py` |
