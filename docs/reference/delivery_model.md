...
當 local orphan 數量較多時，使用唯讀 `./ops/delivery.py branch-review-plan --offset <n> --limit <m>` 分頁取得 `kg.delivery.branch-content-review-plan.v1`。它只選取 branch-audit 已經證明是 local orphan、且沒有 worktree／remote／PR／registry owner claim 的項目，再逐頁取得內容 fingerprint、ahead／behind、bounded paths 與 commit subjects；它不會 publish、discard、建立 owner 或修改 registry。`complete=false` 只要 audit source 仍不完整、尚有未審查頁面或任何內容證據失敗，不能被排程器解讀成清理完成。每個 review item 仍必須回到原 owner publish，或由 operator 以單一 branch 的 fingerprint＋雙重 CAS 明確丟棄。
