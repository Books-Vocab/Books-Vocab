...
- `ios_test.sh`: scoped iOS verification (`unit` default / `--ui` / `--all-targets` / `--file` / `-g` / `--list` / `--launch-benchmark` / `--coverage` / `--coverage-fail-under <percent>` / UI-scope `--dataset <name>`・`--dataset-file <path>` UI World v2 注入（staged `*.scoped.xctestrun` upsert `TestingEnvironmentVariables.KG_FIXTURE_DATASET_B64`；`kg.fixture.dataset.v2` 必須明確宣告所有 top-level domain，並同時帶 data/auth/Keychain token state/entitlements、UserDefaults/iCloud KVS preferences、SwiftData row/source state 與 file-backed asset manifest，malformed seed arg、未知 fixture domain/id、缺 world/key、seed exception 直接 fail hard，auth `keychainTokenState` 明示 `available` / `readFailed` / `absent`，preferences seed 前寫入，Notebook / VocabularyEntry 的 sync/action/archive/reader-exclusion、notebook `isDefault`/`sortOrder`、notebook entry metadata、entry `bookTitle`/`reviewMode`、reviewDeck notebook identity 由 manifest 明示，asset 以 source path + byteSize + sha256 驗證並依 `installAs` materialize 到 app Documents；Bookshelf `bookAssetRef` 指向 `assets.books.*` 並物化到 `Documents/Books/<fileName>`；Catalog podcast episode `durationSec` 由 manifest 明示；Runtime podcast series `sortOrder`、episode `durationSec` / `previewDurationSec` / `download` 由 manifest 明示，`download` 控制 audio/subtitle 是否已下載與 `localAudioPath` / `localSubtitlePath`；E2E 證明測試 `FixtureDatasetUITests`，repo contract `RepoFixtureDatasetsContractTests`）) with shared build lock, heartbeat, preserved failure logs, false-green detection, Xcode build DB lock retry, simulator preboot, dedicated `BooksAndVocabUnitTests` unit scheme + dedicated `BooksAndVocabUITests` UI scheme, cache-first `build-for-testing` / `test-without-building` reuse, explicit `--cache-status|--prepare-cache|--clean-cache` lifecycle controls, UI app launch-profile injection (`--ui-launch-profile standard|ui-smoke` via `KG_UI_TEST_APP_ARGS_JSON`), `prepare-cache` build-log/result-bundle artifacts in `kg.ios.test-cache.v1`, xcresult-derived timing/perf breakdown (`testBodyMs` / `xcresultSessionMs` / `appLaunchAverageMs` / `appLaunchSamples` / `invocationOverheadMs`), and `kg.ios.coverage.v1` line-coverage verdict with lowest-covered production files; downstream `ios_ops.sh runs/snapshot` 會把這些 timing/cache/coverage 欄位提升成第一屏判讀資料
...
- **Catalog preview auth contract**: DEBUG Catalog 的登入狀態由 `CatalogPreviewAuth` 顯式注入；logged-in 必須在 scenario source 明示 `userId` / `token` / `displayName` / `userEmail`，logged-out 必須明示 nil。`CatalogCoverageTests` 會擋 `CatalogPreviewAuth(isLoggedIn: true|false)` 這種短式 constructor 與 preview fallback literal，避免 Catalog 像素被 `AuthManager.shared` 殘留 session 或假 user 資料污染。
- **Catalog preview entitlement contract**: Paywall / Pro Catalog 狀態由 UI World `entitlements.*` seed 驅動；`PaywallScenarios` 透過 `FixtureDatasetStore.requireEntitlementsSeed` 取 `free` / `pro` / `cancelledButActive` / `adminGranted`，缺 seed 直接 fail-fast。`CatalogCoverageTests` 會擋 Paywall Debug source 重新宣告 `KGSubscriptionStatus`、`makeStatus`、admin source literal 或 sync timestamp。
