...
- **snapshot 會順手生 review sidecar**:`catalog snapshots` 成功複製 PNG 後，會在同一個 `out_root` 自動生成 `review.html`（editorial gallery）/ `catalog.html`（atlas 自由縮放 SVG 心智圖，與 CLI `tree/node/node-url` 共用 `nodePath`）/ `review_manifest.json`（含 `tree` 欄位）/ `review_state.json`；JSON payload 的 `review.*` 與文字輸出都會帶路徑。sidecar 生成失敗只記 warning，不會遮蔽 raw PNG 已成功落地。
...
./ops/catalog_review_entry.py current             # payload 含 reviewHtml + canvasHtml
./ops/catalog_review_entry.py serve --port 8787   # 同時暴露 /review.html 與 /catalog.html
