...
`snapshot.datasetFile` 必須是完整 `kg.fixture.dataset.v2` UI World:top-level key set、`datasetID`、asset buckets(`books/audio/images/subtitles/text`)與每個 asset 的 `sourcePath/byteSize/sha256/installAs/contentType` 都在 `load_profile` 階段驗證;source 檔缺失、byteSize/hash 漂移或非 UI World schema 直接 fail-fast,不把錯誤延後到 simulator 或 renderer。
