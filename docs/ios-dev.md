...
- **Scheme**: `BooksBrowser`
- **工作目錄**: `/Users/chenliangyu/Desktop/Development/BooksBrowserWorkspace/`
...
- `references/system_architecture.md` — 完整 iOS ↔ 後端同步協議、認證架構、API 端點規格
# BooksBrowser iOS 架構（開發參考）
...
Reader 查詞
  ↓
VocabularyEntry（SwiftData，syncStatus=0 pending）
  ↓
POST /api/vocab（batchAdd）→ 伺服器生成 embedding + 找 candidates
  ↓
POST /api/pipeline（fire-and-forget）→ 伺服器背景 Enrich/Link/Difficulty/Mochi
  ↓
GET /api/vocab?since=<上次同步時間>（pullCardsToLocal）→ 更新本地 SwiftData
```
...
```swift
// 職責
// - 管理登入狀態：Apple Sign-In / Google Sign-In / Custom Token
// - Keychain 存儲 Bearer Token
// - HTTP 401 → 自動登出 + 清空 SwiftData
// - 登出時：清空 SwiftData + 重置 kg_last_incremental_sync

// Token 作為 Authorization: Bearer <token> 傳給後端
// 後端據此建立 data/users/<user_id>/ 隔離目錄
```
...
```swift
// 主要方法
func batchAdd(words: [String]) async throws -> [CardIdMapping]
func triggerPipeline() async throws
func pullCardsToLocal(since: Date?) async throws -> [CardDTO]
func deleteWord(_ word: String) async throws
func getConfig() async throws -> UserConfig
func updateConfig(_ config: UserConfig) async throws
func quickTranslate(word: String, context: String) async throws -> QuickTranslation
```

### BackgroundSyncActor

```swift
// Swift actor（Thread-safe）
// 在背景 ModelContext 執行，不阻塞主線程
// 三種同步模式：
// - 增量同步：GET /api/vocab?since=<timestamp>
// - 全量同步：GET /api/vocab（無 since 參數）
// - 最終一致性：遍歷本地 cards，確認遠端同步狀態
```

## 資料模型

### VocabularyEntry（SwiftData）

```swift
// 主要欄位：
var word: String
var meaning: String
var pos: String?        // 詞性（由後端 Enrich 補充）
var note: String?       // 教師備注（由後端 Enrich 補充）
var difficulty: Float?  // Zipf 難度（由後端計算）
var difficultyTier: String?  // rare/advanced/intermediate/core
var kgCardId: String?   // 後端 Card ID
var syncStatus: Int     // 0=pending, 1=synced
var actionType: String? // "delete" = 待刪除
var kg_last_incremental_sync: Date?
```
...
```
模式 A：Google Sign-In（推薦）
  iOS Google SDK → Google User ID → Keychain → Bearer Token

模式 B：自訂密語（測試用）
  用戶在 Settings 輸入任意字串 → Keychain → Bearer Token

Guest Mode：
  無 token → 無法 KG 同步 → 可離線查詞 + 本地儲存
  登入後 → 待收錄單字自動推送至 KG
```
...
```
HTTP 401 → AuthManager.logout()
  → 清空 SwiftData 該帳號所有卡片
  → 清空 kg_last_incremental_sync
  → 下次登入強制全量同步
```
...
App
├── SettingsView
│   ├── 登入/登出（Apple/Google/Custom Token）
│   ├── 伺服器位址設定（KGService.baseURL）
│   └── Mochi API Key 設定
│
├── ReaderView
│   ├── EPUB 解析（WKWebView + JS 注入）
│   ├── 單字點擊 → quickTranslate → 顯示翻譯彈窗
│   ├── 收藏 → batchAdd + triggerPipeline
│   └── 底線濃度控制（--vocab-opacity CSS Variable）
│
└── VocabularyView
    ├── 單字列表（難度分層、搜尋）
    ├── KnowledgeGraphView（力導向圖，使用 Grape 套件）
    │   ├── 節點大小 ∝ 難度
    │   └── 邊顏色：同義詞=綠、易混淆=橙、字根=藍
    └── SyncView（手動觸發同步）
```
...
syncStatus=0 (pending)
  ↓ batchAdd 成功
syncStatus=1 (synced)，kgCardId 填入

actionType="delete"
  ↓ deleteWord 成功
從 SwiftData 刪除

is_deleted=true（從 GET /api/vocab 返回）
  ↓ pullCardsToLocal
從本地 SwiftData 刪除
```
...
```
伺服器位址：https://wordnexus.lol
Info.plist NSExceptionDomains：需包含 wordnexus.lol（允許 HTTPS）
模擬器調試：設定 Settings → Knowledge Graph → 伺服器位址
```
