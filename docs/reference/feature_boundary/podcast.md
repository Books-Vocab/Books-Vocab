...
| `PodcastSentenceLevelView.swift` | ~430 | 句級字幕 + 長按整句翻譯 + 點詞查詞 + follow-mode pill `struct PodcastSentenceLevelView: View`（iPhone/iPad：拖曳隱式脫離 follow，pill 僅在脫離時顯示；Mac Catalyst：滑鼠滾輪/觸控板 indirect scroll 不觸發 DragGesture，pill 常駐為明確 toggle「停止跟隨 ⇄ 追隨當前」，邏輯在 `shouldShowFollowControl`）。**逐字底線 + 自動置中捲動為連續引擎(非離散)**：current 句逐詞底線是**單一 capsule**,由 `TimelineView(.animation(paused: !isPlaying))` 每幀以 `PodcastPlaybackClock.projectedTime` 外推 `viewModel.playbackAnchor`(15Hz tick + seek/play/pause/rate 刷新)→ `PodcastWordProgress.locate` → `PodcastUnderlineGeometry.bar` 連續定位(同行詞間 lerp、跨行不橫拉);word 幾何走 `WordFrameKey` anchorPreference,rects 每 layout pass 解析一次、TimelineView 只 lerp(不重排 `CachedFlowLayout`)。取代了舊「per-word `.overlay(Capsule)` + `.animation(value: highlightedWordIndex)`」的逐字 snap(在 <130ms 短詞會 strobe)。`wordFollowEnabled==false` 時 overlay closure 不畫底線。自動捲動由 `.onChange(of: playbackAnchor)`(15Hz)算 `PodcastScrollGeometry.sentenceFraction` → `proxy.scrollTo(currentId, anchor: PodcastScrollGeometry.centerAnchor(fraction))` 包 `.linear(0.1)`,當前句隨句內進度連續上爬過中心(取代每句 discrete `.center` 跳);沿用既有 `ScrollViewReader`/`isFollowing`/Catalyst toggle 不變(刻意不重寫成 offset-driven 容器:iOS 17 無 offset-based scrollPosition + LazyVStack realization/a11y 風險)。連續引擎的純函式幾何在 `PodcastPlayerMetrics.swift`(沿 `PodcastSeekBarGeometry` 範式,單測 `PodcastPlaybackGeometryTests`) |
