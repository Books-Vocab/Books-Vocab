...
- **已接線（Figma 真注入）**：`AppRadius` / `AppSpacing` scale（`s1`–`s10`/`hairline`/`micro`/`tiny`）/ `AppFonts.TypeScale` / `AppFonts.Tracking` / `AppElevation`（z0–z4 opacity·blur·y）/ `AppMotion` duration（quick·control·chip·progress）·spring（standard·emphasized·press·content-reveal·modal-swap 的 response/damping）·`TapFeedback`（scaleDown·opacityDip）已改為引用 `DesignTokens.*`。在 Figma 改這些 → Push → `npm run build` 重生 `DesignTokens.swift` → **重編 app 即生效**，無需手改 Swift。
- **未接線（仍須手動）**：**全部顏色**（`AppColors` 原色 + `AppTheme` 三主題語意色，精確 float + `WCAGContrastTests` 釘死對比，刻意不自動接）、`AppMotion` 的 **easing**（tokens 是 cubic-bezier 字串，iOS `timingCurve` 收 4×Double + 無對應的 0.4 duration，型別不橋接）/ **transition**（複合 `AnyTransition`）/ 無 token 對應的 spring 成員（`relaxedSpring`/`buttonSpring`/`swipe*Spring` 等）、`AppFonts.LineSpacing`（與 web `type.leading` 語意不同）、`AppSkin` 組合層。改這些須**手動改對應 Swift**，再讓 drift check 確認 tokens.json 對齊。
