...
| `catalog_review_cli.py` | catalog review / UI asset gallery CLI：`summary`/`show`/`list`/`stats`/`report` 查 artifact 與單張資產、`mark`/`apply` 寫 review state、`verify`/`repair`/`doctor` 做 sidecar 健康檢查與操作建議，另有 `hero`/`coverage`/`cleanup` shortcut 視角。輸入是 `review_manifest.json + review_state.json`，主用途是離線審圖、批量標記、以及對 `review.html` 內的 `assetID/permalink` 做機器可讀操作。測試：`ops/tests/test_catalog_review.py` |
