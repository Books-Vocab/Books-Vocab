...
- **Mac Catalyst** (macOS 15.0+): iOS app runs natively on Mac via Catalyst (`SUPPORTS_MACCATALYST`, App Sandbox); 原生視窗尺寸(最小 900×640 / 首發 1100×760 置中)+ Reader 沉浸閱讀(EPUB/PDF 隱藏 title bar,`MacWindowChrome`); Cmd+F search shortcut (`VocabularyListPresenter`); 指標 hover 回饋(卡片浮起 `appHoverLift` / 可點列染色 `appHoverRowTint`) + 可拖曳分隔線欄寬游標(`MacColumnResizeCursor`,`UIPointerInteraction`); podcast series 右鍵/長按追蹤切換
