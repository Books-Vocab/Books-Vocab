...
**收尾的坑（會遇到，不是意外）**：批次整合過的分支清不掉，而**兩道拒絕的理由不同**——`sweep` 對「工作樹還在」的分支一律 KEEP（那與包含性無關），`resolve` 才是被包含性判準擋下。後者的判準是「這分支動過的每個檔，main 現在是不是就是這分支的版本」；而
...
> **範圍**：本段講的是**彼此獨立的 session**——各自帶著自己的 intent、各自 cutover，衝突點在共享的 primary。同一批 fan-out 出去的被派者**不適用**：那批不各自 cutover，見上方「批次整合」與 `docs/sop/agent_org.md`「交回狀態」段。兩者都叫並發，處置相反。
