...
**Topology: local-`main`-centric, three planes.** Local `main` is the trunk; worktrees fork from it and `cutover` fast-forwards it OFFLINE (develop plane). `origin/prod` is the deploy target — `deploy` advances it (release plane) and the felix reconciler, watching `origin/prod`, turns a backend delta into a rollout; `origin/main` is now the backup mirror (`sync` pushes it, **no production effect** — the reconciler ignores main). So local main runs ahead of both origins until a deliberate `deploy`. Worktree health is a **process-internal invariant**, not a periodic cleanup chore. Isolated worktrees are driven by the `worktree-flow` skill orchestrating `ops/worktree_orchestrate.py` (`preflight` / `open` / `adopt` / `gate` / `catchup` / `land` / `integrate` / `close-wave` / `cutover` / `resolve` / `sync` / `deploy` / `sync-main` / `freeze`). Normal sessions assume concurrent peers and stop at commit + `hand-back`, leaving the worktree active; batch assembly before authorization must use `integrate --commit --no-gate` and hand back the integration tree. Only a session with the user's current, explicit statement that no other agent/session is working and authorization to gate + cutover may run the final Gate and enter the develop plane, after which the landed worktree self-cleans via `resolve`. That develop authorization does not authorize `sync`, `deploy`, or `release`; backup and release each require their own explicit intent. (The former `converge` / `cleanup` / `promote` sweep methodology is retired.) Three-plane verb semantics: `docs/sop/release.md`.
**`close-wave` is the Delivery Team coordinator's resumable batch closure entrypoint.** After the normal develop authorization, it composes the existing integration, fresh Gate, cutover, source resolve, backlog anchor, validation, and final integration-tree resolve primitives in that order; it refuses foreign active worktrees and never runs `sync` or `deploy`. Before that authorization, use `integrate --commit --no-gate` and hand back as specified by `worktree-flow`.
