...
PROMISE_RULES: list[tuple[str, tuple[str, ...]]] = [
    ("Read", ("Reader", "PDF Reader", "Translation", "TOC", "Selection Toolbar", "Vocab Highlight Picker")),
    ("Connect", ("Knowledge Graph", "Linked Card", "Add Link", "Graph Link", "Link Reason", "Collocation Explain")),
    ("Retain", ("Today Review", "Review Calendar", "Pending Vocab", "Vocab Heatmap", "Vocab Forecast", "Vocab Calendar", "Progress Capsule", "Review Fold", "Sync View")),
    ("Continue", ("Podcast", "Bookshelf", "Book Card", "Notebook", "Archived Vocab", "Vocabulary List", "Stats View")),
]
...
def classify_promise(category: str) -> str:
    for promise, needles in PROMISE_RULES:
        if any(needle in category for needle in needles):
            return promise
    return "Weak"


def collect_items(source_root: Path) -> list[dict]:
    items: list[dict] = []
    for path in sorted(source_root.rglob("*.png")):
        ...
        promise = classify_promise(category)
        items.append(
            {
...
def main() -> int:
    ...
    items = collect_items(source_root)
