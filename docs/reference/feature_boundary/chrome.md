...
| `icons/` | 16/48/128 PNG |
| `_locales/zh_TW/messages.json` | chrome.i18n 訊息 SoT（含 notebook scope/management/appearance sheet、`首輪 $time$` / `下次 $time$` / `$count$ 個連結` placeholder 替換、加詞 outbox 的 `同步中…`/`待重試`/`重試`）；manifest `name`/`description` 走 `__MSG_*__`，`default_locale: "zh_TW"` |
...
| **對標 iOS 視覺** | 跑 `ops/chrome_parity.sh --audit`。contact sheet 只當全局概覽：`tools/shots.mjs` headless 截 9 個 UI case（content light/dark/sepia · outbox failed row · notebook sheet · detail · options · empty · error），逐 case 注入 in-page mock 走 app.js 真實 render path；`tools/compare.mjs` 按 parity manifest 與 `~/Desktop/IOS截圖參考/`（`IOS_REF_DIR` override）並排成總覽圖。細節檢查以 `tools/parity-audit.mjs` 產物為準：逐張 `diff.png` 看像素差異、`zoom.png` 放大比對 row/header/card 區塊、`palette.txt` 比較平均色與 dominant colors、`metrics.json`/`summary.json` 追 RMSE/MAE/SSIM/pHash drift。對標 IMG_8954（單字本列表）/8955（單字詳情）/8957（設定）；outbox failed row/notebook sheet/empty/error/dark/sepia 為 Chrome 特有或待補 reference 的 case。產物 git-ignored |
