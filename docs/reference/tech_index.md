...
| `ios_test.sh` | iOS test runner;預設 unit target,支援 `--ui` / `--all-targets` / `--file` / `-g` / `--list`;先走 `simulator ensure-booted`,再用 cache-first `build-for-testing`/`test-without-building` 重用 `.cache/ios-test-derived-data`,verdict JSON 會寫 `timings.bootMs/buildForTestingMs/testInvocationMs/xcodebuildMs/totalMs` 與 `cache.status`;仍保留共享 build lock、heartbeat、`Test.xcresult` + raw log + `[ios][tests]` summary,false-green 執行數優先取官方 `.xcresult`、raw log fallback,build DB lock retry |
