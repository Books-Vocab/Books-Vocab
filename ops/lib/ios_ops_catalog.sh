...
cmd_catalog_snapshots_json() {
  local out_root="$1" destination="$2" groups_csv="${3:-}" scenarios_csv="${4:-}"
  ...
  local fixture_root="" derived_data_root="" base_xctestrun="" scoped_xctestrun=""
  local build_cmd="" scope_test_cmd="" use_scoped_xctestrun=0
  ...
  xcode_log="$(mktemp)"
  xcode_err="$(mktemp)"
  build_cmd="xcodebuild build-for-testing -project $XCODEPROJ -scheme $CATALOG_SCHEME -destination $destination -parallel-testing-enabled NO -test-timeouts-enabled NO OTHER_SWIFT_FLAGS=\$(inherited) -DKG_RUN_CATALOG_SNAPSHOTS"
  scope_test_cmd="xcodebuild test-without-building -xctestrun <scoped-xctestrun> -destination $destination -parallel-testing-enabled NO -test-timeouts-enabled NO -only-testing:BooksBrowserTests/CatalogSnapshotTests"
  ...
  if [[ -n "$groups_csv" || -n "$scenarios_csv" ]]; then
    ...
    use_scoped_xctestrun=1
    build_cmd="xcodebuild build-for-testing -project $XCODEPROJ -scheme $CATALOG_SCHEME -destination $destination -parallel-testing-enabled NO -test-timeouts-enabled NO -derivedDataPath $derived_data_root OTHER_SWIFT_FLAGS=\$(inherited) -DKG_RUN_CATALOG_SNAPSHOTS"
  ...
  if [[ "${KG_IOS_OPS_FIXTURE:-}" == "1" ]]; then
    ...
    if [[ "$use_scoped_xctestrun" -eq 1 ]]; then
      ...
      if catalog_cached_products_ready "$derived_data_root" "$base_xctestrun"; then
        ...
        : >"$xcode_log"
        : >"$xcode_err"
        scoped_xctestrun="$(dirname "$base_xctestrun")/$(basename "${base_xctestrun%.xctestrun}").scoped.xctestrun"
        catalog_prepare_scoped_xctestrun "$base_xctestrun" "$groups_csv" "$scenarios_csv" "$scoped_xctestrun"
        test_cmd="xcodebuild test-without-building -xctestrun $scoped_xctestrun -destination $destination -parallel-testing-enabled NO -test-timeouts-enabled NO -only-testing:BooksBrowserTests/CatalogSnapshotTests"
        if catalog_run_scoped_xctestrun "$scoped_xctestrun" "$destination" "$xcode_log" "$xcode_err"; then
          rc=0
        else
          rc=$?
          cache_status="stale"
      ...
      if [[ "$cache_status" != "hit" || "${rc:-1}" -ne 0 ]]; then
        if catalog_rebuild_scoped_cache "$destination" "$derived_data_root" "$xcode_log" "$xcode_err"; then
          ...
          if [[ -z "$base_xctestrun" ]]; then
            ...
            scoped_xctestrun="$(dirname "$base_xctestrun")/$(basename "${base_xctestrun%.xctestrun}").scoped.xctestrun"
            catalog_prepare_scoped_xctestrun "$base_xctestrun" "$groups_csv" "$scenarios_csv" "$scoped_xctestrun"
            test_cmd="$build_cmd && xcodebuild test-without-building -xctestrun $scoped_xctestrun -destination $destination -parallel-testing-enabled NO -test-timeouts-enabled NO -only-testing:BooksBrowserTests/CatalogSnapshotTests"
            if catalog_run_scoped_xctestrun "$scoped_xctestrun" "$destination" "$xcode_log" "$xcode_err"; then
              rc=0
...
cmd_catalog_snapshots() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      -h|--help|help)
        echo "Usage: ./ops/ios_ops.sh catalog snapshots [--out-root <dir>] [--destination <xcodebuild-destination>] [--group <category>]... [--scenario <category/title>]... [--json]"
        return 0
        ;;
      *)
  ...
  payload="$(cmd_catalog_snapshots_json "$out_root" "$destination" "$groups_csv" "$scenarios_csv")" || rc=$?
  if (( json )); then
    printf '%s\n' "$payload"
    return "$rc"
  fi
