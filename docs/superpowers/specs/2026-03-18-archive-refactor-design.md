...
涉及檔案：
- `ios/BooksBrowser/Views/Vocabulary/Scenes/NotebookListView.swift` — 新增 toolbar item + sheet
...
- `ios/BooksBrowser/Views/Vocabulary/Scenes/KGVocabView.swift` — 移除 `showArchiveList` state 及其 sheet binding（避免雙入口）；`handleArchiveTap` swipe action 保留
...
涉及檔案：
- `backend/src/kg/vocab_service.py` — `archive_vocab_word` 增加 graph + cards_store 參數及邏輯；`build_links_by_kind` 過濾 `is_archived` 卡片
...
2. **A↔B 連結，A 和 B 都被封存**：兩者各自 deprecate 一次（冪等，已 deprecated 的不重複計數）。先解封存 A 時，B 仍是 archived，A↔B 不恢復。B 也解封存後，`restore_links_for(B)` 檢查到 A 已非封存狀態，連結恢復為 active。正確行為。
...
1. **`vocab_graph.py` — `embed_and_link_new_cards`**：`find_similar` 回傳的 `other_id` 在 `add_candidate` 前檢查 `!is_archived`。
2. **`pipeline_service.py` — `_step_embed`**：`cards.all()` 補 embedding 時跳過 archived 卡片。
3. **`pipeline_service.py` — `_step_link`**：candidate pair 判斷時加上 `card.is_archived` 檢查（與現有 `is_deleted` 並列）。

涉及檔案：
- `backend/src/kg/vocab_graph.py`
- `backend/src/kg/pipeline_service.py`

### 7. `build_links_by_kind` 過濾封存卡片

`vocab_service.build_links_by_kind` 構建 `CardResponse.linksByKind` 時只跳過 `is_deleted`。需加上 `is_archived` 過濾，讓封存卡片不出現在其他卡片的連結列表中。

涉及檔案：
- `backend/src/kg/vocab_service.py` — `build_links_by_kind` 第 66 行條件加上 `or other_card.is_archived`
