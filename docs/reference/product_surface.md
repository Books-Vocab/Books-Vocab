...
- **iOS ops 統一控制面**(`ops/ios_ops.sh`): agent/human 優先入口,組合 Xcode 官方 CLI 與既有 primitives；`build`/`test`/`archive` 委派底層腳本,`archives` 查 Organizer,`issues` 解析 xcresult/log,`logs` 拉 runtime log,`doctor` 做 release readiness,`workflow release` 產出發版下一步,`runs`/`reports` 讀最近 build/test verdict + log/xcresult artifact,`snapshot`/`dashboard` 一次拉 project/Organizer/TestFlight/readiness/workflow/runs。read-only surface 皆有文字輸出,agent 用 `commands --json`(`kg.ios.commands.v1` 自描述 side-effect/delegate/schema catalog)、`logs --json`(`kg.ios.logs.v1`)、`doctor --json`(`kg.ios.doctor.v1`)、`workflow release --json`(`kg.ios.workflow.v1`)、`runs --json`(`kg.ios.runs.v1`)、`snapshot --json`(`kg.ios.snapshot.v1`;`--include-logs` 時內嵌 `kg.ios.logs.v1`)。
