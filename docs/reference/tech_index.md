...
| `ios_test.sh` | iOS test runner;預設 unit target,支援 `--ui` / `--all-targets` / `--file`(`.swift` 可省、裸型別名亦可,多檔同名報錯) / `-g` / `--list` / `--launch-benchmark` / `--cache-status` / `--prepare-cache` / `--clean-cache` / `--ui-launch-profile <standard|ui-smoke>` / `--device <udid|name>` / `--destination '<dest>'` / `--lease`(自動租 pool 模擬器,並行 agent 用);unit scope 走 dedicated `BooksBrowserUnitTests` scheme，UI scope 走 dedicated `BooksBrowserUITests` scheme；`--launch-benchmark` 固定跑 `BooksBrowserUITests/testLaunchPerformance`；runner 先走 `simulator ensure-booted`,再用 cache-first `build-for-testing`/`test-without-building` 重用 `.cache/ios-test-derived-data`;UI profile 透過 `KG_UI_TEST_APP_ARGS_JSON` 注入 `XCUIApplication.launchArguments`;verdict JSON 會寫 `timings.lockWaitMs/deviceRunLockWaitMs/bootMs/buildForTestingMs/testInvocationMs/testBodyMs/xcresultSessionMs/xcresultHarnessOverheadMs/appLaunchAverageMs/appLaunchSamples/invocationOverheadMs/xcodebuildMs/totalMs` 與 `cache.status`;`kg.ios.test-cache.v1` 現也會回 `artifacts.buildLog/resultBundle`，讓 `prepare-cache` 失敗時能直接追 build-for-testing 證據；cache JSON schema 會回 `productsReady` / `xctestrunPath` / `buildForTestingMs`;第一屏 diagnostics 另外印 `[ios][test-timing] ...` 與有量測時的 `[ios][perf] metric=AppLaunch ...`;**細粒度鎖**(`/tmp/kg-ios-build.lock` 只在 `build-for-testing` 持有,test 執行不持鎖,並行 agent 各自 leased 模擬器同跑)外,同一台 simulator 的 `test-without-building` 還會經 per-device execution lock 序列化,避免 warm-cache 同機互撞；agent/worktree/CI 若仍想用共享預設 simulator，runner 會 fail-fast，要求 `--lease` / `--device` / `--destination`（單機除錯才可 `KG_IOS_TEST_ALLOW_SHARED_SIM=1` opt-out）；lease slot 綁 owner pid/token,live run 不會被 TTL reclaim,cleanup 也不會誤刪別人的 slot；完成 sentinel `.kg-test-cache-complete` 擋 half-written cache、heartbeat、`Test.xcresult` + raw log + `[ios][tests]` summary,false-green 執行數優先取官方 `.xcresult`、raw log fallback,build DB lock retry；`rebuild-after-failure` 只處理 `.xctestrun` / test-runner 類 cache/infrastructure failure,不會把真 `TEST FAILED` 洗成綠燈 |
