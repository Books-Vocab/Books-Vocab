...
- 全批視覺人工 attestation 完成後，才用 `ios_ui_review_matrix.py record-many --summary <run-many.json>` all-or-nothing 回寫；它重新驗證 source HEAD、UI World hash、manifest coverage、machine contract 與最新 `review_state.json`。長時間 build/test 期間須繼續做不競爭 lock 的 source／fixture／report 工作，保留 PID heartbeat，不得只等待輸出。
