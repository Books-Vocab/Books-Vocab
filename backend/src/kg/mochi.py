"""Mochi API adapter for syncing Cards."""
            ...
            try:
                ...
                if e.response.status_code in (429, 500, 502, 503, 504) and attempt < 3:
                    ...
                    if self.retry_callback:
                        self.retry_callback(f"Mochi status {e.response.status_code}, retrying in {wait}s...")
                    time.sleep(wait)
                    continue
                raise e
    ...
    def pull_updates(self, dry_run: bool = False) -> dict:
        ...
        for mochi_id, local_id in iterator:
            ...
            if "collocations" in updates:
                ...
                # Sort to compare content only? No, order might matter.
