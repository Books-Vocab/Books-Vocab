...
| `shared/icons.test.js` | — | inline SVG glyph brace-match anti-drift：守門 content.js（isolated world inline）與 `shared/icons.js` 兩份一致 |
...
| `icons/` | 16/48/128 PNG |
| `_locales/zh_TW/messages.json` | chrome.i18n 訊息 SoT（含 notebook scope、`首輪 $time$` / `下次 $time$` / `$count$ 個連結` placeholder 替換、加詞 outbox 的 `同步中…`/`待重試`）；manifest `name`/`description` 走 `__MSG_*__`，`default_locale: "zh_TW"` |
