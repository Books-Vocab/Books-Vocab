...
| `ios_test.sh` | iOS test runner;預設 unit target,支援 `--ui` / `--all-targets` / `--file` / `-g` / `--list`;共享 build lock,heartbeat,輸出 `Test.xcresult` + raw log + `[ios][tests]` summary,false-green 執行數優先取官方 `.xcresult`、raw log fallback,build DB lock retry |
