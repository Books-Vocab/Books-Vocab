...
resp = client.chat.completions.create(
    model="gpt-5.5",   # 或 "gpt-5.3-codex"；ChatGPT Plus 訂閱實測可用（2026-05-23）
    messages=[{"role": "user", "content": "..."}],
    stream=True,
)
```

> **模型權限注意**（2026-05-23 ChatGPT **Plus** 實測）：
> - ✅ 可用：`gpt-5.5`、`gpt-5.3-codex`
> - ❌ 不開放：`gpt-5`、`gpt-5-codex`、`gpt-5.1-codex`、`gpt-5.4-codex`、`gpt-5.5-codex`、`gpt-5.5-thinking`（回 `not supported when using Codex with a ChatGPT account`，即使 `/v1/models` 顯示出來）
>
> 升 Pro / Business / Enterprise 才有更廣範圍。
