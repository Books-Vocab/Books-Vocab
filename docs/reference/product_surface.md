...
- `ios_test.sh`: scoped iOS verification (`unit` default / `--ui` / `--all-targets` / `--file` / `-g` / `--list`) with shared build lock, heartbeat, preserved failure logs, false-green detection, Xcode build DB lock retry, simulator preboot, dedicated `BooksBrowserUnitTests` unit scheme + dedicated `BooksBrowserUITests` UI scheme, cache-first `build-for-testing` / `test-without-building` reuse, and explicit `--cache-status|--prepare-cache|--clean-cache` lifecycle controls
