...
- 報告型 P1–P15 任務另用 `ops/fixtures/ios_ui_review_clusters.json` 固定五個架構 cluster 與每列 exact selector；先跑 `ios_ui_review_clusters.py validate/expand-run-plan`，再由 `ios_ui_run_many.py run` 一次準備 cache、逐 selector 產生獨立 stable bundle。共用 selector 可只執行一次但保留所有 requirement mapping；matrix 的 `requiredFixtureIDs` 必須指向 UI World 的 canonical top-level fixture 或 `scenarioContext` dictionary/surface-contract fixture（測試 runtime argument 不是 fixture ID）；summary 的 `passed_unattested` 只代表 machine contract，不能直接改 matrix。
- `ios_ui_run_many.py run` 會在 build 前建立唯一 `run-manifest.json`，將 prepare/selector 的 log、xcresult、step screenshot、review page、video 與 stable bundle 收在同一個 batch staging root，並以 parent batch ID + execution ID 綁定 PID、HEAD、worktree、branch、Simulator、UI World 與方法快照。helper 完成 normalization 後只保留 stable bundle；不要把 selector PASS 當成 visual 或 requirement PASS。失敗／中止 run 由 manifest 記為 `failure`／`abandoned`，先 dry-run 再用 `ios_ui_run_many.py cleanup --staging-root ... --commit` 回收；該命令只處理 TTL 已過期的失敗 staging，且 recorded PID、xcodebuild／runner consumer 或 known lock 未確認清空時 fail-closed，保留 manifest、methods、summary 與 cleanup receipt。
...
- 全批視覺人工 attestation 完成後，才用 `ios_ui_review_matrix.py record-many --summary <run-many.json>` all-or-nothing 回寫；它重新驗證每個 bundle 的 source HEAD、UI World hash、manifest／machine contract／最新 `review_state.json`，再把同一 requirement 的多個 exact selector 做 state union。每個 logical required/counterexample state 必須在 union 中恰好對應一個獨立 asset；缺 state、重複 state 或 required/counterexample 共用 asset 都零寫入。若舊 capture 名稱與產品語意不同，只能在 matrix 明列 `stateAliases`，禁止任意 substring matching。`record-many` 後只允許提交 tracked matrix receipt；後續 validate 可接受 source commit 的祖先 HEAD，但唯一例外路徑是 `ops/fixtures/ios_ui_review_matrix.json`。任何 report、source/test/docs drift、dirty tree、無法解析 Git HEAD/cleanliness、非祖先 source 或改寫 provenance 都必須重新執行 evidence。報告在 evidence 前更新並固定；不可用報告收據掩蓋 evidence 後的變更。長時間 build/test 期間須繼續做不競爭 lock 的 source／fixture／report 工作，保留 PID heartbeat，不得只等待輸出。
