...
- `resume-published --mode maintenance` 是同一個 exact owner／branch／HEAD／base／Scope／queue／hold preflight 下的窄修復入口；若 GitHub 尚未產生 `required` context，它可以接受唯一、明確的 `ABSENT` required observation，讓原 owner 修復 gate transport 或 workflow。這只授權 bounded local repair，絕不把 PR 視為 required-green、merge-ready 或可 dispatch；修復後仍必須重新產生 required check 與 typed hand-back。`required-failure` mode 及有 context 但狀態不一致的 evidence 仍 fail closed。
