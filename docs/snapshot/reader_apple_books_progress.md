...
> **本工作一律在專屬 worktree `/Users/chenliangyu/kg-worktrees/reader-polish` 操作**（主 dir `~/project/kg` 會被其他 session 切換 branch）。

## 已完成（branch `reader-apple-books-polish`，全部 compile + adversarial review PASS）
...
行為 / UI / log 待**實機驗證**（sandbox 無 PTY 跑不了 xcodebuild test / 目視）。
...
- 全在 worktree `/Users/chenliangyu/kg-worktrees/reader-polish`。
- 編譯：`./ops/ios_build.sh`（shlock 多 worktree 安全；只能 compile-verify，行為/UI 實機交使用者）。
