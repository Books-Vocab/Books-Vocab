...
- 梳理後跑 `./ops/backlog.py validate` 與 `./ops/backlog.py list --ungroomed`,回報佇列剩幾筆(這是 kaizen 迴圈唯一的進度指標)。另回報 `list --missing-brief` 的筆數——**未解但缺 `brief`/`scope` 的回填債**,即看板上只能顯示 agent 散文的那些票。它是**回填佇列不是 dispatch 佇列**:多數已有完整 plan,要補的是一句話不是修法,當 dispatch 派出去只會開出一個沒東西可改的工作樹。已結案的不計入(不上看板)。另回報 `list --acceptance-manual` 的筆數——那是「這批梳理裡有多少條沒有機器能驗」,一個會慢慢長大就代表梳理在退化的數字。**梳理的另一半是判準本身會不會說謊**:`./ops/backlog.py audit-criteria`(唯讀)實跑未解票的 `acceptance_cmd`,今天就綠的列為候選——要嘛是殭屍票,要嘛是判準不驗它宣稱的東西(而收案閘跑的就是同一條命令)。**它會執行 store 裡的自由文字**,所以裸呼叫會被工具直接拒絕:先 `--dry-run` 看會跑什麼,再用 `--filter` / `--limit` 決定你願意在這台機器上跑什麼,真要全跑就明示 `--all`。綠是**候選不是判決**,天生會綠的判準用 `update <id> --acceptance-green-expected '<理由>'` 具名豁免(可清點:`list --acceptance-green-expected`)。**`red` 也不是健康證明**——判準因為不相干的理由失敗(grep 檔案不見回 2、`pytest -k` 選不到回 5)一樣落在那裡,而那是沒人捲到底的長桶。桶的語意、逾時判定與這個盲點的量測見 `docs/reference/tech_index.md` 的 `backlog.py` 列。
