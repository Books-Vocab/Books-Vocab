...
def test_unknown_agent_profile_rejected_before_any_state_change():
    # kimi was retired 2026-08-09 — a stale --agent-profile must fail loudly,
    # not silently fall back to the claude account.
...
def test_claude_profile_keeps_legacy_model_env(monkeypatch):
    ...
    # A Claude Code-spawned shell already exports ANTHROPIC_BASE_URL; clear it
    # so the assert below measures what _agent_subprocess_env *injects*.
...
def test_agent_sidecar_round_trip(tmp_path):
    ...
    pipeline.configure_agent(profile="claude", model="sonnet")
    pipeline.write_agent_sidecars(ws)

    assert pipeline.read_agent_sidecars(ws) == ("claude", "sonnet")
