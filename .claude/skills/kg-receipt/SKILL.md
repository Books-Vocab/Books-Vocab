...
- **stream 決定 owner,所以填錯等於沒人追**:`--stream IMP` 由 `platform-steward` 追到 resolved;`--stream APP` 由對應 Line worker(`ios-engineer` / `backend-engineer`)追到 resolved,其收件匣是 `./ops/backlog.py list --stream APP`。判準見上方 Checklist 的「Stream 分流」。
