...
- **Tests**（`backend/tests`）：API contract、robustness、admin/test-matrix、auth provider tests（Apple/Google）、text_utils/enrich modules、pipeline lock-queue concurrency tests、Sentry init scrubbing
- **Tests**（`ios/BooksBrowserTests`，66 tests）：vocabulary entry lifecycle、bilateral link mutation、reader bridge planner、session persistence、notebook orphan defense（resolveNotebookId 7 + sanitizeOutbox 8 + triggerPipelinesIsolated 3 = 18 tests）、PodcastVocabContext (6) + ReaderTranslationHandler (10) + ReaderVocabularyCapture (7)
