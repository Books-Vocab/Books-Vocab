...
def _stub_s3(existing_keys: set[str] | None = None) -> MagicMock:
    """A MagicMock S3 client whose head_object raises for keys not in
    existing_keys (so skip-existing + reconcile can be exercised)."""
    ...
    s3.exceptions.ClientError = _NoSuchKey
