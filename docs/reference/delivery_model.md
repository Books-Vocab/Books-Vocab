...
`metrics.reanchor_required` 只計現有 `LaneState.REANCHOR`，大於 0 時另輸出 `reanchor_front`；controller 不在此重新解釋 REANCHOR 分類語義。
