...
**自我提升迴圈**:工具 / CLI / 文檔 / 架構摩擦走 andon → receipt 強制表態(規則見 `kg-receipt`「Tooling Debt」)→ `docs/runbook/backlog/<id>.json`(SoT,一筆一檔;入口一律 `ops/backlog.py`,**禁止手改** `docs/runbook/improvement_backlog.md`——那是 render 產出)→ `platform-steward` triage 到 resolved(連 commit = 可回溯)。無聲妥協(硬幹)違鐵律9;升級階梯見 `docs/sop/agent_org.md`。
