...
- UI World decode 階段會先驗 asset manifest 跨引用：Runtime podcast series/download 只能指 `assets.audio.*` / `assets.subtitles.*`，Reader 只能指 `assets.text.*` / `assets.books.*`，Bookshelf 非空 `bookAssetRef` 只能指 `assets.books.*`；ref 不存在或 bucket 不對直接 `DecodingError`，不是 seed/install 時的 late failure。
