...
- Today Review 翻卡狀態訊號：back identifier 必須放在 `backContentMounted` 的真內容分支（`TodayReviewPresenter+CardContent.swift` 的 `todayReview.card.back`）。answer fold surface 連同其 `accessibilityLabel("翻譯：…")` 在正面/摺疊（height 0）時仍常駐 view tree——identifier 放 surface/Group 上會讓 `.exists` 對翻卡狀態說謊。評分後 progress 用 `waitUntilLabelContains("k / N")` 等 fling 落定；計數 badge 斷言用「·1」避免裸 `1` 誤配。
