...
write_state() {
  local free="$1" prev="$2" growth="$3" active="$4" cache="$5" docker_cache="$6" docker_running="$7" verdict="$8" reason="$9" action="${10}"
  local dir tmp
  dir="$(dirname "$STATE_FILE")"; mkdir -p "$dir" 2>/dev/null || return 1
  tmp="$STATE_FILE.$$.$RANDOM.tmp"
  printf '{"schema":"kg.disk.guard.v1","host":"%s","free_bytes":%s,"previous_free_bytes":%s,"growth_bytes":%s,"active_build":%s,"cache_kb":%s,"docker_cache_kb":%s,"docker_active":%s,"verdict":"%s","reason":"%s","action":"%s","at":"%s"}\n' \
    ...
    "$verdict" "$reason" "$action" "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" > "$tmp" || { rm -f "$tmp"; return 1; }
  mv -f "$tmp" "$STATE_FILE"
}

main() {
  local free prev growth active cache docker_cache docker_running free_gib growth_gib docker_gib verdict reason action
