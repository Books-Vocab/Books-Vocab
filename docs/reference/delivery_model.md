...
- `resume-published --mode maintenance` 是同一個 exact owner／branch／HEAD／base／Scope／queue／hold preflight 下的窄修復入口；若 GitHub 尚未產生 `required` context，它可以接受唯一、明確的 `ABSENT` required observation，讓原 owner 修復 gate transport 或 workflow。當既有 exact owner path 仍在 previous hand-back、而 PR branch 已由同一 owner 以 fast-forward 推進時，command 只在 previous hand-back 是 remote HEAD 的 ancestor、worktree clean、branch/path/Scope/CAS 全相符時，以 `git merge --ff-only` 將該原 path 前進到 exact remote HEAD；非 fast-forward、dirty、path drift 或任何 identity mismatch 都 fail closed。這只授權 bounded local repair，絕不把 PR 視為 required-green、merge-ready 或可 dispatch；修復後仍必須重新產生 required check 與 typed hand-back。`required-failure` mode 及有 context 但狀態不一致的 evidence 仍 fail closed。
- 若 published PR 的 required 已是 exact `FAILURE`，但失敗原因是新 `origin/main` 已包含的基線修復，原 owner 可明確使用 `reanchor --allow-required-failure` 進行同 owner 的本地 recovery。這個選項只放行 exact OPEN／main／mergeable／無 hard hold／未入 native queue／typed receipt identity 相符的 required failure，並重新建立以 current live main 為 base 的本地 worktree；它不宣稱 merge-front、不跳過 required、不授權 queue／merge，也不操作 GitHub。完成後仍須跑 bounded tests、產生新 hand-back、由 PI 重跑 required，通過後才回到一般 CM merge-front 規則。缺少 required、required 非 failure、receipt／owner／head／Scope／CAS 不一致時仍 fail closed。
...
`.github/workflows/pr-readiness.yml` 先用 `ops/delivery.py validate-pr-body` 驗證 PR body 只含一份合法 `kg.delivery.handback.v1` machine receipt，並把 receipt 綁到 exact PR HEAD；workflow 不自行重寫另一套 regex schema。`.github/workflows/pr-gate.yml` 的 workflow `pr-gate` 會產生短、可重現的 `required` check run；它只回答這個 PR 是否滿足 repository 基線，不代表所有受影響 domain 都已完整驗證。若 exact published PR 的 required 為 `ABSENT` 或 `FAILURE`，PI 可用 `trigger-required` 重新執行同一 HEAD 上既有的 `pull_request` workflow run；command 會先重讀 unique PR mapping、registry receipt、body、paths、base／HEAD 與目前 check status，再以 branch、HEAD、event 與最新建立時間精確選取既有 run，並拒絕 workflow dispatch run、其他 HEAD、進行中的 run 或已成功的 run。若選到的 exact `pull_request` run 是 `queued` 且建立已超過 15 分鐘，command 可對同一 run ID 執行 supported force-cancel，再重新讀取同一 exact identity；只有 readback 已是 terminal non-success 才可 rerun。若 cancel 回覆表示 run 已完成、但同一 exact readback 暫時仍為 active，command 只對該明確競態做有限次重讀；讀回 terminal non-success 才可 rerun，仍 active、近期 queued、`in_progress`／其他 active status 或其他 cancel/readback 不一致都 fail closed，不以 cancel exit code 代替 GitHub 狀態證據，也不另造 workflow run。這確保新的 `required` 結果仍屬於 PR 的 branch-protection status context，而不是只在 commit API 留下一個 PR 看不見的 observation。找不到可重跑的 exact `pull_request` run 時 fail closed，不用不具 PR 關聯的 manual dispatch 充數；這個修復不清除 hold，也不產生 merge eligibility。
