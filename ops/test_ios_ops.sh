...
echo "$catalog_json" | jq -e --arg root "$catalog_tmp/build/snapshots" --arg persistRoot "$catalog_persist_root" '.schema=="kg.ios.catalog.v1" and .action=="snapshots" and .status=="ok" and .mode=="fixture" and .artifacts.root==$root and .artifacts.pngCount==2 and (.artifacts.paths|length)==2 and (.scope.groups|length)==0 and (.scope.scenarios|length)==0 and all(.artifacts.paths[]; startswith($root)) and (.validation.status=="ok") and .validation.actualPngCount==2 and .validation.uniformImageCount==0 and .validation.transparentImageCount==0 and .validation.minPixelWidth==16 and .validation.maxPixelWidth==16 and .validation.minPixelHeight==16 and .validation.maxPixelHeight==16 and (.validation.images|length)==2 and (.validation.transparentImages|length)==0 and (.validation.warnings|length)==0 and .cache.status=="not-applicable" and .cache.key==null and .cache.root==null and (.test.command|contains("CatalogSnapshotTests")) and .test.exitCode==0 and .copy.exitCode==0 and .copy.containerDataPath=="/tmp/kg-sim-fixture/container" and .review.status=="ok" and (.review.canvasHtml|endswith("/catalog.html")) and (.review.reviewManifest|endswith("/review_manifest.json")) and .workspaceArtifact.status=="ok" and (.workspaceArtifact.root==$persistRoot) and (.workspaceArtifact.name|startswith("catalog-full-")) and (.workspaceArtifact.artifactRoot|startswith($persistRoot)) and (.workspaceArtifact.canvasHtml|endswith("/catalog.html")) and (.workspaceArtifact.reviewManifest|endswith("/review_manifest.json")) and .flags.compileTimeFlag=="KG_RUN_CATALOG_SNAPSHOTS"' >/dev/null \
  && ok "catalog snapshots --json exports fixture PNG artifacts" || fail_t "catalog snapshots --json invalid: $catalog_json"
[[ -f "$(echo "$catalog_json" | jq -r '.review.canvasHtml')" ]] && [[ -f "$(echo "$catalog_json" | jq -r '.review.reviewManifest')" ]] \
  && ok "catalog snapshots auto-generates atlas sidecar" || fail_t "catalog atlas sidecar missing: $catalog_json"
[[ -f "$(echo "$catalog_json" | jq -r '.workspaceArtifact.canvasHtml')" ]] && [[ -f "$(echo "$catalog_json" | jq -r '.workspaceArtifact.reviewManifest')" ]] \
  && ok "catalog snapshots persists atlas artifact into workspace snapshot root" || fail_t "catalog workspace artifact missing: $catalog_json"
