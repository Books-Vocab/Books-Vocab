...
| `PodcastSentenceLevelView.swift` | ~560 | 句級字幕 + 長按整句翻譯 + 點詞查詞 + follow-mode pill `struct PodcastSentenceLevelView: View`（iPhone/iPad：拖曳隱式脫離 follow，pill 僅在脫離時顯示；Mac Catalyst：滑鼠滾輪/觸控板 indirect scroll 不觸發 DragGesture，pill 常駐為明確 toggle「停止跟隨 ⇄ 追隨當前」，邏輯在 `shouldShowFollowControl`）。**逐字底線為連續引擎(非離散);自動捲動為原生 ScrollView 離散 follow**：current 句逐詞底線是**單一 capsule**,由 `TimelineView(.animation(paused: !isPlaying))` 每幀以 `PodcastPlaybackClock.projectedTime` 外推 playhead → `PodcastWordProgress.locate` → `PodcastUnderlineGeometry.bar` 連續定位(同行詞間 lerp、跨行不橫拉);word 幾何走 `WordFrameKey` anchorPreference,rects 每 layout pass 解析一次、TimelineView 只 lerp(不重排 `CachedFlowLayout`)。取代了舊「per-word `.overlay(Capsule)` + `.animation(value: highlightedWordIndex)`」的逐字 snap(在 <130ms 短詞會 strobe)。`wordFollowEnabled==false` 時 underline closure 不畫底線。<br>**🟦 bubble 進度填充背景(取代二元 active crossfade)**:current 句 bubble 容器底色**整塊當進度條**——背景抽 `bubbleBackground`,基底靜態 `tint.08`,其上 `activeBubbleFill` 疊一層**第二個 per-frame `TimelineView(.animation(paused: !isPlaying))`**:讀同一 `liveAnchor` 外推 playhead → `PodcastBubbleFill.fillFraction(playhead:start:end:)` → 左錨 `Rectangle` mask 寬 = `geo.width * frac`,`tint.18` 隨播放進度左→右連續填充(非 modifier 動畫,從 clock 讀故不滯後 playhead);取代舊「active `.18` / 其餘 `.08` 二元 + `contentFade` 換句硬切」(使用者反映焦點「在跳」)。**僅 gate `isCurrent`(1-cell);`isSelecting` 時改靜態滿版 `.18`(選詞不隨 playhead 漂)**。⚠️**刻意不 gate `wordFollowEnabled`**——該 toggle 是「逐詞 follow 底線」,此 fill 是「逐句進度焦點」,不同 affordance,關底線時仍亮。焦點交棒(current ⇄ 非 current)crossfade 走 `podcastBubbleHandoff`(decelerate 0.4s 與捲動同調),取代 `contentFade` 硬切。<br>**📌 結論:current cell 現掛 2 個 per-frame `TimelineView`** —(1) underline overlay(gate `wordFollowEnabled`)(2) bubble fill background(gate `isCurrent` 恆開);兩者共用 `liveAnchor` + `paused: !isPlaying`,per-frame 成本鎖在單一 current cell,其餘 cell 靜態、per-cell `Equatable` 短路與 scroll-freeze 護欄不受影響。<br>**↕️ 句內逐行 follow(P3:長句橫跨多行不再紋風不動)**:follow 粒度由「句」下沉到「行」。current cell 每幀經 `lineFollowTarget(t:rects:)` 算「當前詞所在行最左詞」(`PodcastLineFollow.anchorWordIndex`,minY 容差分組 + index tie-break),包成 `PodcastLineFollowTarget(sentenceId,anchorWordIndex)` 經 **`PodcastLineFollowKey` preference 每幀上報**;preference **掛在穩定的 `Color.clear` leaf**(非條件 underline 內容、非 EmptyView——附 preference 於退化 view 為脆弱契約,對齊 `PodcastProgressTicker` 高頻通道模式),故**不受 `wordFollowEnabled` gate**(僅 current cell 回非 nil)。`onPreferenceChange(PodcastLineFollowKey)` → `handleLineFollow`:**僅「同句 + 跨行(anchorWordIndex 變)」**才 `withAnimation(podcastFollowScroll(duration: PodcastFollowScroll.lineDuration=0.35s)) { scrollTo(wordScrollID, anchor:0.4) }`(同行不動、換句交 `followScroll`,二來源不互打);word 全域 scroll id 由 `PodcastLineFollow.wordScrollID("<sid>.<idx>")` 掛 current/next cell 的 word `.id` 作 target。**雙 scroll 來源**:句界 `onChange(of: currentId)` → `followScroll`(自適應 duration)+ 行界 `onPreferenceChange` → `handleLineFollow`(0.35s)。自動捲動走**原生 `ScrollView` + `ScrollViewReader` + `LazyVStack`**(僅 realize 在屏氣泡):句界 follow = `onChange(of: currentId)` → `followScroll`,duration 由 `PodcastFollowScroll.duration(sentenceDuration:)` 依該句時長**自適應**(短句短捲動避免被下次 boundary 半路打斷)、曲線 `AppMotion.podcastFollowScroll(duration:)` 改 **decelerate `(0.1,0.7,0.2,1.0)` 快進柔出**(去除舊 `(0.65,0,…)` 起步加速段=使用者感的「阻尼感」),**每句界一次 animated `scrollTo`**(離散,非每幀 offset)。**follow anchor 由 `.center` → `UnitPoint(0.5,0.4)`**(當前行/句釘 viewport 40%,長句下方留展開空間;`.task` 進場置中 / `followScroll` / `handleLineFollow` 統一)。進場置中走 `.task`(等 LazyVStack realize 再 `scrollTo`;`onAppear`/`onChange` 對 mount 當下值會 no-op);episode swap 由 `onChange(of: PodcastTranscriptIdentity(sentences:))` 重置中;re-engage 翻 `isFollowing` flag → `onChange(of: isFollowing)` 動畫捲回。手動瀏覽走**原生捲動**(GPU 合成、原生慣性、lazy realize);iPhone/iPad 輕量 `simultaneousGesture(DragGesture(minimumDistance:8))` 只翻 `isFollowing=false`;**Mac Catalyst 觸控板 indirect scroll 不觸發 DragGesture → pill 常駐為明確 toggle**(此點不變)。<br>**⚡ scroll-freeze 根治(根因史)**:舊「offset-driven 連續置中」引擎(非 lazy VStack + per-sentence `GeometryReader`/`SentenceCenterKey` 在 named `transcriptSpace` 量中心 + 外層 `TimelineView(.animation)` 每幀算 `PodcastScrollGeometry.centerOffset` 套 `.offset` + 自訂 `DragGesture`/`clampOffset`/`seedManualOffsetFromFollow`)**進場即凍**——三段真機單變數實驗(30 句仍凍 / 拿掉 TimelineView 警告消失 / 換原生 ScrollView 流暢)定位根因為 offset+per-frame-TimelineView 引擎本身,**非整集實體化成本**。整個 offset 引擎連同上述符號與 `PodcastScrollGeometry`(centerOffset/sentenceFraction)+ `PodcastScrollGeometryTests` 已刪。<br>**`.equatable()` 保留但語境改變**:token 樹仍抽成 `private struct PodcastTranscriptColumn: View, Equatable` 並 `.equatable()` 包起,其 `==` 比 **O(1) `PodcastTranscriptIdentity`(count + firstStart + lastEnd)** + sentence-level `currentId` / `selectionState` / `subtitleSize` / `wordFollowEnabled` / `isPlaying`,**刻意排除 playhead**——現用途是防 token-irrelevant 父重評(如 `isFollowing` flag 翻動)+ episode swap 偵測,而非舊「外層 follow TimelineView 每幀跳過」。playhead 走 reference 型 `PodcastLiveAnchor`(`viewModel.liveAnchor`,`playbackAnchor.didSet` 單點鏡像、`@ObservationIgnored`),underline 內層 `TimelineView` 每幀讀 `liveAnchor.value` 取最新 playhead,即使父 Equatable body 被跳過也不凍。`speakerSlots` 在 `body` hoist 一次;speaker→tint 抽 `PodcastSpeakerTint`(column 與 pill 共用)。`PodcastTranscriptIdentity` 純函式單測 `PodcastTranscriptIdentityTests` |
...
| 檔案 | 行數 | 說明 |
|------|------|------|
| `PodcastPlayerMetrics.swift` | ~120 | 播放器 feature-local 尺寸常數 + 多個純函式 helper（皆有對應純函式單測 `PodcastPlaybackGeometryTests`）：<br>• `PodcastSeekBarGeometry` — track position → time / progress width / buffered width<br>• `PodcastFollowScroll` — 句界 follow `scrollTo` **自適應捲動時長** `duration(sentenceDuration:)`（coverage 0.85、clamp `[minDuration 0.25, maxDuration 0.6]`）+ 行界 follow 用 `lineDuration`（0.35s，行位移小、長句快速跨多行不互打）<br>• `PodcastBubbleFill` — current bubble 進度填充 `fillFraction(playhead:start:end:)`（clamp [0,1]、zero-length 句不除零）<br>• `PodcastLineFollow` — 句內逐行 follow 錨點：`anchorWordIndex(wordRects:activeIndex:)`（當前詞所在行最左詞 index，minY 容差分組 + index tie-break）+ `wordScrollID(sentenceId:wordIndex:)`（word 全域 scroll id `"<sid>.<idx>"`） |
...
- **新增字幕呈現邏輯** → `PodcastSentenceLevelView` 或 `PodcastSubtitleView`,layout 計算抽到 `CachedFlowLayout` 或 `SubtitleRenderState`。**per-frame `TimelineView` 隔離契約**:任何「隨 playhead 連續變化」的呈現(底線 / bubble 進度填充 / 未來新效果)**只能掛在 current cell**(gate `isCurrent`,讀 `liveAnchor` + `paused: !isPlaying`),其餘 cell 必須靜態——否則 per-cell `Equatable` 短路與 scroll-freeze 護欄被架空。current cell 現已有 **2 個** per-frame TimelineView(underline overlay + bubble fill),再加須評估 per-frame 成本。**preference 上報契約**:逐幀上報的 scroll/follow target(如 `PodcastLineFollowKey`)**必須掛在穩定 leaf(`Color.clear`)**,絕不附在條件 view / EmptyView / 退化 Group(脆弱契約);消費端 `onPreferenceChange` 靠 `Equatable` target 自然只在真正變化(跨行 / 換句)時 fire,維持離散 animated `scrollTo`、不落每幀 offset 路徑。**多 scroll 來源不互打**:句界(`onChange currentId`→`followScroll`)與行界(`onPreferenceChange`→`handleLineFollow`)須各管其責(後者僅同句跨行才動),勿讓兩條 animated scroll 互相追逐
