...
- **App Store Connect 查詢/改文案**(`ops/asc.sh`): 主體 codemagic CLI 包裝;`versions`/`builds`/`metadata`/`info`/`review-status` 唯讀查版本與審查狀態,`review-detail`/`screenshots` 走 raw 旁路 `ops/asc_get.py`(審查備註/demo 帳號、截圖逐張 state),`set <field> <value>` 改 version-localization 文案(description/keywords/urls/whats-new/promo,預設 dry-run、`--yes` 才真寫、空值被擋);`set-review <field> <value>` 改 appStoreReviewDetail(送審備註 notes/demo 帳號/聯絡人,codemagic 未暴露 → 走 raw PATCH `ops/asc_patch.py`,同 dry-run、`--yes` gate),`set`↔`set-review` 跨物件互相指路;被拒原因 Resolution Center 文字 API 不可讀(須 GUI);不做 submit-for-review。細節見 `docs/sop/ios.md §發版`
