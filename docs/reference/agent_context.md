...
| **Delivery Team Integrator** (`delivery-coordinator`) | `kg-agent-context`、assigned batch、backlog lifecycle、`worktree-flow` 的停止點／批次整合／`close-wave`／並發段、`docs/sop/release.md` | Ticket Factory 的 triage 細節、未被 batch 指向的 domain 文件、Catalog 與其他 team 的內部狀態 | 先查 assigned ticket 的 `fix_site` 與 registry trigger；日常依 Gate tier 只跑所需深度，跨功能根目錄才升 S3；非重大 S2/S3 BLOCK 可用具名既有 ticket 帶過，重大或 S0/S1/S4 BLOCK 退回；Gate BLOCK 時依 hand-back 的 exact source thread ID 退回原提交者；只在 state／collision／primary race 等不穩定狀況傳 thread message |
