...
- **Podcast API（分層授權 guest/free/pro）**: `/api/podcasts*`（手刻 Range/206 音訊串流 + `ep_num` Path 驗證；S3 模式 audio 副檔名由 metadata `audioFormat` 決定，相容 legacy mp3）。**訪客可瀏覽** list/detail/cover（`get_current_user_optional`）；`audio` 端點依 tier gate — guest→`401 {code:auth_required}`（須登入才能播）、free→只能聽每 series **ep1 的 `preview.*` 試聽片**（其餘 `403 {code:upgrade_required}`）、pro→full audio；policy 在 `podcast_access.py`（牆統一、無 per-series 旗標）。free 試聽走**獨立預生成 `preview.*` 物件**（progressive MP4 單 moov 無法乾淨 byte 截斷）。+ series 封面 proxy `GET /api/podcasts/{sid}/cover`（image/png，缺則 404 → client 退程序化封面）+ per-user podcast progress LWW SQLite store（legacy 無認證 `/api/podcast-media/` 掛載已於 2026-05 移除）。生成 pipeline 工程文檔:`docs/sop/podcast_pipeline.md`
