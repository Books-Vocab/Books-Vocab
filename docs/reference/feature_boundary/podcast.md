...
| `PodcastSentenceLevelView.swift` | ~430 | 句級字幕 + 長按整句翻譯 + 點詞查詞 + follow-mode pill `struct PodcastSentenceLevelView: View`（iPhone/iPad：拖曳隱式脫離 follow，pill 僅在脫離時顯示；Mac Catalyst：滑鼠滾輪/觸控板 indirect scroll 不觸發 DragGesture，pill 常駐為明確 toggle「停止跟隨 ⇄ 追隨當前」，邏輯在 `shouldShowFollowControl`）。**逐字底線 + 自動置中捲動為連續引擎(非離散)**：current 句逐詞底線是**單一 capsule**,由 `TimelineView(.animation(paused: !isPlaying))` 每幀以 `PodcastPlaybackClock.projectedTime` 外推 `viewModel.playbackAnchor`(15Hz tick + seek/play/pause/rate 刷新)→ `PodcastWordProgress.locate` → `PodcastUnderlineGeometry.bar` 連續定位(同行詞間 lerp、跨行不橫拉);word 幾何走 `WordFrameKey` anchorPreference,rects 每 layout pass 解析一次、TimelineView 只 lerp(不重排 `CachedFlowLayout`)。取代了舊「per-word `.overlay(Capsule)` + `.animation(value: highlightedWordIndex)`」的逐字 snap(在 <130ms 短詞會 strobe)。`wordFollowEnabled==false` 時 overlay closure 不畫底線。自動捲動為 **offset-driven 連續置中**(非 `scrollTo`):`transcriptColumn` 為**非 lazy** VStack(LazyVStack 的 realization window 不隨 `.offset` 移動會 blank,故不可用),每句用 background `GeometryReader` 在自有 named coordinateSpace(`transcriptSpace`)報中心 Y(offset-independent);follow 時 `TimelineView(.animation(paused:))` 每幀以 `PodcastScrollGeometry.centerOffset` = `viewportH/2 − lerp(當前句中心, 下一句中心, 句內進度)` 連續 `.offset` 位移,當前句滑過正中、跨句界靠插值零跳(`centerOffsetIsContinuousAcrossBoundary` 鎖死)。不 follow 時同一 offset 由自訂 `DragGesture` 驅動,離開 follow(drag / 長按選詞 / Catalyst 停止 pill)都先 `seedManualOffsetFromFollow` 無縫接手、`clampOffset` 防拖出空白。**權衡**:manual browse 無原生慣性;**Mac Catalyst 觸控板 indirect scroll 不觸發 DragGesture → 手動瀏覽退化**(follow 自動捲動正常、pill 可停跟隨);非 lazy 全列 VStack 對超長 transcript 有記憶體/初次 layout 成本(待評估 windowing)。純函式幾何在 `PodcastPlayerMetrics.swift`(沿 `PodcastSeekBarGeometry` 範式,單測 `PodcastPlaybackGeometryTests`) |
