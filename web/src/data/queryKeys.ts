 ...
 *
 * 設計：每個 domain 一個命名空間，`all` 是 list/collection key，detail / 過濾型 key
 * 以 `all` 為前綴。如此 `invalidateQueries({ queryKey: queryKeys.notebooks.all })`
 * 會 partial-match 命中該 domain 的所有 query（list + 各 detail），mutation 後一次
 * 失效、自動重抓。新增 query 一律從此檔取 key，禁止在 hook 內手寫字串陣列（否則
 * invalidation 會因 key 漂移而漏命中）。
 *
