...
|---|---|---|---:|---|---|
| Lightsail Instance | AWS `967512079054` / `booksbrowser-kg-api-2gb` | `small_3_0`(2 GB RAM / 60 GB SSD / 3 TB transfer) | **$12.00** | Fixed bundle | `aws lightsail get-instances` |
...
| S3 backup | AWS `967512079054` / `kg-backups-prod-967512079054` | Standard,daily tar+gz | **~$0** | Usage(預估首年 < $1/月,容量極小)| `aws s3 ls --summarize` / `aws ce` |
...
**$15/月 = 月底之前一定會繳的底**。LLM 是「用越多繳越多」,單獨追蹤(§2)。
