...
- **Catalyst/iPad 2D 可堆疊 block workspace** (regular 主路徑): podcast(series→episodes→player 水平 Miller columns)與 vocab(word detail / today review 開欄)導航從各自 `safeAreaInset` 假兩欄改成一套 2D workspace 引擎(`Platform/PanelWorkspace/`);每欄/block 自帶 ✕ 關閉(關欄串聯收右側、欄空收欄)、可拖曳分隔線調欄寬/block 高、`drawerReveal`/`statusRowReveal` 過場動畫;regular→compact 翻轉收欄。iPhone compact 行為零改動(沿用既有 push / sheet)
